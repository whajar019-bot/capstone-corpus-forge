...
def insert_document(filename: str, filetype: str, content: str, db_path: str = DB_PATH) -> int:
    """
...
def get_active_documents(db_path: str = DB_PATH) -> List[Dict[str, Any]]:
    ...
    try:
        cursor.execute('SELECT * FROM documents WHERE is_active = 1 ORDER BY upload_date DESC')
        rows = cursor.fetchall()
        return [dict(row) for row in rows]
    except sqlite3.Error as e:
...
def toggle_document_active(document_id: int, db_path: str = DB_PATH) -> bool:
    ...
    try:
        ...
        # Toggle the status
        new_status = 1 - row[0]
        cursor.execute('UPDATE documents SET is_active = ? WHERE id = ?', (new_status, document_id))
        conn.commit()
        return True
    except sqlite3.Error as e:
        conn.rollback()
...
def insert_artifact(artifact_type: str, content: Dict[str, Any], document_id: int, 
    ...
    if artifact_type not in ('flashcard', 'quiz', 'code_review'):
        raise ValueError(f"Invalid artifact type: {artifact_type}. Must be 'flashcard', 'quiz', or 'code_review'")
...
def get_artifacts_by_type(artifact_type: str, db_path: str = DB_PATH) -> List[Dict[str, Any]]:
    """
...
def insert_usage_record(request_count: int = 0, token_count: int = 0, 
                       db_path: str = DB_PATH) -> int:
    """
...
def get_document_by_id(document_id: int, db_path: str = DB_PATH) -> Optional[Dict[str, Any]]:
    """
...
def get_artifacts_by_document_id(document_id: int, db_path: str = DB_PATH) -> List[Dict[str, Any]]:
    """
